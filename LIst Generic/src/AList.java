...
class AList<E> implements List<E> {
   ...
   public void prev() { 
      if (curr != 0) curr--; } // Move left
   public void next() { 
      if (curr < listSize) curr++; } // Move right
   public int length() { 
      return listSize; }      // Return list size
   public int currPos() { 
      return curr; }         // Return current position
   ...
   public int find(E it) {
      for(int i=0; i<listSize; i++) // Shift them down
         if(listArray[i].equals(it))
            return i;
      return -1;
